...
int main(int Arguments, char* Argument[])
{
  if (Arguments != 2)
  {
    ...
    fprintf(stderr, "Advent2024_10_1 <.dat file>\n");
    exit(1);
  }
  ...
  for (X=0; X<InputXLen; X++)
    ...
    for(;;)
      ...
      if (++Choice[ChoiceNr] < 4)
      {
        ...
        switch (Choice[ChoiceNr])
        {
          ...
          case 3: Xpath[ChoiceNr]--;  break;
				...
				{
					Sum++;
					// Mark this 9 as an underscore, so that it doesn't count twice
					Input[Ypath[ChoiceNr]][Xpath[ChoiceNr]] = '_';
      ...
      else
      {
		...
		// Finally, go over the grid again, to switch all _ markers back to 9
		for (y=0; y<InputYLen; y++)
			for (x=0; x<InputXLen; x++)
				if (Input[y][x] == '_')  Input[y][x] = '9';
	} /* for(X) */
