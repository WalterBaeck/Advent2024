...
int main(int Arguments, char* Argument[])
{
  if (Arguments != 2)
  {
    ...
    fprintf(stderr, "Advent2024_08_1 <.dat file>\n");
    exit(1);
  }
  ...
  for (Index=0; Index<62; Index++)
  {
    ...
    for (FirstPos=0; FirstPos<NrOfPos[Index]-1; FirstPos++)
      for (SecondPos=FirstPos+1; SecondPos<NrOfPos[Index]; SecondPos++)
      {
        ...
        // For this pair of antennas, determine its two antinodes
        int DiffX,DiffY;
        DiffX = FirstX-SecondX;  DiffY = FirstY-SecondY;
        // Antinode beyond the First antenna
        X = FirstX + DiffX;  Y = FirstY + DiffY;
        if ((X>=0) && (X<InputXLen) && (Y>=0) && (Y<InputYLen))  Input[Y][X]='#';
        // Antinode beyond the Second antenna
        X = SecondX - DiffX;  Y = SecondY - DiffY;
        if ((X>=0) && (X<InputXLen) && (Y>=0) && (Y<InputYLen))  Input[Y][X]='#';
      }
  } /* for (Index) */
