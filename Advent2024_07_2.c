...
int main(int Arguments, char* Argument[])
{
  if (Arguments != 2)
  {
    ...
    fprintf(stderr, "Advent2024_07_2 <.dat file>\n");
    exit(1);
  }
  ...
  ////////////////////////////    O p e r a t i o n    //////////////////////////
    ...
    // New operator
    unsigned long long Concatenation(unsigned long long LHS, unsigned long long RHS)
    {
      char NumStr[40];
      unsigned long long RetVal;
      sprintf(NumStr, "%llu%llu", LHS, RHS);
      if (1 != sscanf(NumStr, "%llu", &RetVal))
      {
        fprintf(stderr, "Concatenation on InputLine #%d: %llu || %llu into %s cannot grok\n",
            InputLineNr, LHS, RHS, NumStr);
        exit(4);
      }
      return RetVal;
    }
    ...
    for(;;)
    {
      ...
      if (++Choice[ChoiceNr] < 3)
      {
        ...
        switch (Choice[ChoiceNr])
        {
          ...
          case 2: Result[ChoiceNr] = Concatenation(LeftHandSide,Input[ChoiceNr+1]); break;
        }
#ifdef DEBUG
