...
int main(int Arguments, char* Argument[])
{
  ...
  char Digit[5];
  int DigitNr;
  char Seq[3][100][100], ThisSeq[100], NextSeq[100];
  int SeqNr, NrOfSeq[3], ThisPos, ThisLen, NextPos;
  int MulSum = 0;
  ...
  int MinLength, TotalLength, Score;
  ...
  for (DigitNr=0; DigitNr<4; DigitNr++)
  {
    ...
    for (Yfirst=0; Yfirst<2; Yfirst++)
    {
      ...
      NextSeq[NextPos++] = 'A';
      // Close this NextSeq string and save it, if not found before
      NextSeq[NextPos] = '\0';
    ...
    // // // // // Go through 2 layers of DirPad possibilities for this Digit // // // // //

    for (SeqNr=0; SeqNr<2; SeqNr++)
    {
      NextSeqNr = 0;
...
#ifdef DEBUG
    printf("  -Found %d level-%d sequences:-\n", NrOfSeq[SeqNr+1], SeqNr+1);
    for (NextSeqNr=0; NextSeqNr<NrOfSeq[SeqNr+1]; NextSeqNr++)
    {
      ...
      for (ThisPos=0,Score=0; ThisPos<strlen(Seq[SeqNr+1][NextSeqNr]); ThisPos++)
        Score += ScoreFromTo(ThisPos ? Seq[SeqNr+1][NextSeqNr][ThisPos-1] : 'A', Seq[SeqNr+1][NextSeqNr][ThisPos]);
    ...
    int MinLength = 0;
    for (NextSeqNr=0; NextSeqNr<NrOfSeq[2]; NextSeqNr++)  
      if ((!MinLength) || (strlen(Seq[2][NextSeqNr]) < MinLength))
        MinLength = strlen(Seq[2][NextSeqNr]);
