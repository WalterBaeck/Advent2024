    ...
    for (SeqNr=0; SeqNr<NR_ROBOTS-1; SeqNr++)
    {
...
#ifdef DEBUG
        printf("  From SeqNr[%d] Current[%d] {%d} with Len %d NrOfChain %d\n",
...
#ifdef DEBUG
          printf("   Seq[%d][%d][%d]=%s has length %d\n",
...
#ifdef DEBUG
        printf("    Copying EvolveAct[%2d] with NrOfChain=%d {%d} Len=%d into Next[%d] at Chain[%d]\n",
...
#ifdef DEBUG
      printf("   Next[%d] {%d} len %d NrOfChain %d\n",
      ...
      for (NextNr=0,NextPtr=Next; NextNr<NrOfNext; NextNr++,NextPtr++)
      {
        ...
        for (ChainNr=0,ChainPtr=NextPtr->Chain; ChainNr<NextPtr->NrOfChain; ChainNr++,ChainPtr++)
        {
          ...
          {
            // Try to make a choice between both Acts by considering how they'll evolve
            if      (EvolveAct[ChainPtr->Act[0]].Score > EvolveAct[ChainPtr->Act[1]].Score)
            {
              ChainPtr->Act[0] = ChainPtr->Act[1];
              ChainPtr->NrOfAct = 1;
            }
            else if (EvolveAct[ChainPtr->Act[0]].Score < EvolveAct[ChainPtr->Act[1]].Score)
              ChainPtr->NrOfAct = 1;
#ifdef DEBUG
            if (ChainPtr->NrOfAct == 1)
              printf("    Reduced bloated Chain[%d][%d][%d] to Act[%2d] %s\n",
...
#ifdef DEBUG
        printf("   Next[%d] NrOfChain %d NrOfSplitChains %d\n",
